...
const buildApiUrl = (api: IApi, input: any): URL => {
  ...
  // Forward dynamic arguments (e.g. city, seats, limit, category, etc.) generated by the model
  // as query parameters for GET/HEAD/DELETE calls if they weren't explicitly pre-configured.
  const method = (api.method || "GET").toUpperCase();
  if (method === "GET" || method === "HEAD" || method === "DELETE") {
    ...
    for (const [rawKey, val] of Object.entries(allInputValues)) {
      if (!rawKey || internalKeys.has(rawKey)) continue;
      if (rawKey.startsWith("$")) continue;
      const key = cleanParameterKey(rawKey);
      if (!key) continue;
      if (endpointContainsParameter(endpointTemplate, key)) continue;
