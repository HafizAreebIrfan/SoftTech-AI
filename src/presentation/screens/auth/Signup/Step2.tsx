...
import { useAuthStore, ApiConnection } from "../../../../infrastructure/store/authStore";
import { saveCompanyApiDetails } from "../../../../adapters/api/authApi";
import { DatabaseIcon, TrashIcon, SlidersIcon, TerminalIcon, ServerIcon, KeyIcon, LockIcon, Plus, LeftArrowIcon } from "../../../../assets/icons";
...
const SignupStep2: FC = () => {
  ...
  const { mutate: stepTwoMutate, isPending: isStepTwoPending } = useMutation({
    mutationFn: ({ id, apis }: { id: string; apis: any[] }) => saveCompanyApiDetails(id, apis),
    onSuccess: (res) => {
      if (res && res.success) {
  ...
  const handleStepTwoSubmit = () => {
    ...
    for (const api of apisList) {
      ...
      try {
        ...
        showToast(`Invalid URL format in Endpoint URL for "${api.apiName}".`, "warning");
        return;
      }
      ...
      if (api.apiAuthType === 'API Key') {
        if (!api.apiAuthHeader?.trim()) {
          showToast(`API Key Header Name is required for "${api.apiName}".`, "warning");
          return;
        }
        if (!api.apiCredentials?.trim()) {
          showToast(`API Key Value is required for "${api.apiName}".`, "warning");
          return;
        }
      }
      if (api.apiAuthType === 'OAuth 2.0') {
        if (!api.oauthTokenUrl?.trim()) {
          showToast(`OAuth 2.0 Token URL is required for "${api.apiName}".`, "warning");
          return;
        }
        try {
          new URL(api.oauthTokenUrl);
        } catch (e) {
          showToast(`Invalid OAuth Token URL format for "${api.apiName}".`, "warning");
          return;
        }
        if (!api.oauthClientId?.trim()) {
          showToast(`OAuth 2.0 Client ID is required for "${api.apiName}".`, "warning");
          return;
        }
        if (!api.apiCredentials?.trim()) {
          showToast(`OAuth 2.0 Client Secret is required for "${api.apiName}".`, "warning");
          return;
        }
      }
      ...
      if (api.apiHeaders && api.apiHeaders.trim()) {
        try {
          ...
          showToast(`Invalid JSON structure in Custom Headers for "${api.apiName}".`, "error");
          return;
        }
      }
      ...
      if (!api.apiQueryParams || !api.apiQueryParams.trim()) {
        showToast(`Query Parameters are required for "${api.apiName}".`, "warning");
        return;
      }
      try {
        JSON.parse(api.apiQueryParams);
      } catch (e) {
        showToast(`Invalid JSON structure in Query Parameters for "${api.apiName}".`, "error");
        return;
      }
    }
    ...
    const apisPayload = apisList.map(api => {
      return {
        ...
        // Send raw auth details directly for backend processing:
      ...
      <div className={`${styles.signupcard} mt-4`} style={{ background: colors.BackgroundSecondary, border: `1px solid ${colors.CardBorder}`, borderLeft: `4px solid ${colors.CardActiveBorder}`, boxShadow: `0 10px 40px ${colors.HeaderBoxShadow}` }}>
        ...
        {apisList.map((api, index) => (
          <div
            ...
            style={{ background: colors.Background, border: `1px solid ${colors.CardBorder}`, boxShadow: `0 4px 12px ${colors.OverlayShadow}` }}
          >
            ...
            <div className={`relative p-8 md:p-12`}>
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-8 space-y-6">
                  <div className={styles.formField}>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="md:col-span-2">
                        <label className="font-label text-[10px] uppercase tracking-widest font-bold mb-2 block" style={{ color: colors.TextBody }}>
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            value={api.apiName}
                            onChange={(e) => updateApiField(api.id, 'apiName', e.target.value)}
                            className={`block w-full pl-12 pr-4 py-3 rounded-xl outline-none transition-all text-sm font-label`}
                            style={{ background: colors.BackgroundSecondary, borderColor: colors.CardBorder, color: colors.TextBody }}
                          />
                        </div>
                      </div>
                      <div>
                        <label className="font-label text-[10px] uppercase tracking-widest font-bold mb-2 block font-medium" style={{ color: colors.TextBody }}>
                        ...
                        <div className="relative">
                          ...
                          <select
                            value={api.apiMethod}
                            onChange={(e) => updateApiField(api.id, 'apiMethod', e.target.value as any)}
                  ...
                  <div className={styles.formField}>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        onChange={(e) => updateApiField(api.id, 'apiEndpoint', e.target.value)}
                        className={`block w-full pl-12 pr-4 py-3 rounded-xl outline-none transition-all text-sm font-label`}
                        style={{ background: colors.BackgroundSecondary, borderColor: colors.CardBorder, color: colors.TextBody }}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          onChange={(e) => updateApiField(api.id, 'apiAuthType', e.target.value)}
                    ...
                    <div className={styles.formField}>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={(e) => updateApiField(api.id, 'apiCredentials', e.target.value)}
                              className={`block w-full pl-12 pr-4 py-3 rounded-xl outline-none transition-all text-sm font-label`}
                              style={{ background: colors.BackgroundSecondary, borderColor: colors.CardBorder, color: colors.TextBody }}
                            />
                          </div>
                      ...
                      {api.apiAuthType === 'API Key' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div>
                            <label className="font-label text-[10px] uppercase tracking-widest font-bold mb-2 block" style={{ color: colors.TextBody }}>
                              API Key Header Name <span className="text-red-500">*</span>
                            </label>
                            <div className="relative">
                              <span className="absolute left-4 top-3.5">
                              ...
                              <input
                                ...
                                onChange={(e) => updateApiField(api.id, 'apiAuthHeader', e.target.value)}
                                className={`block w-full pl-12 pr-4 py-3 rounded-xl outline-none transition-all text-sm font-label`}
                                style={{ background: colors.BackgroundSecondary, borderColor: colors.CardBorder, color: colors.TextBody }}
                              />
                            </div>
                      ...
                      {api.apiAuthType === 'OAuth 2.0' && (
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div className="md:col-span-1">
                            <label className="font-label text-[10px] uppercase tracking-widest font-bold mb-2 block" style={{ color: colors.TextBody }}>
                              Token / Grant URL <span className="text-red-500">*</span>
                            </label>
                            <div className="relative">
                              <span className="absolute left-4 top-3.5">
                                <ServerIcon size={18} color={colors.IconColor} />
                              ...
                              <input
                                ...
                                onChange={(e) => updateApiField(api.id, 'oauthTokenUrl', e.target.value)}
                                className={`block w-full pl-12 pr-4 py-3 rounded-xl outline-none transition-all text-sm font-label`}
                                style={{ background: colors.BackgroundSecondary, borderColor: colors.CardBorder, color: colors.TextBody }}
                              />
                            </div>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              ...
                              <input
                                ...
                                onChange={(e) => updateApiField(api.id, 'oauthClientId', e.target.value)}
                                className={`block w-full pl-12 pr-4 py-3 rounded-xl outline-none transition-all text-sm font-label`}
                                style={{ background: colors.BackgroundSecondary, borderColor: colors.CardBorder, color: colors.TextBody }}
                              />
                            </div>
                  ...
                  <div className={styles.formField}>
                    <label className="font-label text-[10px] uppercase tracking-widest font-bold mb-2 block" style={{ color: colors.TextBody }}>
                      Custom Headers (JSON string) <span className="text-slate-500 font-medium">(Optional)</span>
                    </label>
                    <div className="relative">
                      ...
                      <textarea
                        ...
                        onChange={(e) => updateApiField(api.id, 'apiHeaders', e.target.value)}
                        className={`block w-full pl-12 pr-4 py-3 rounded-xl outline-none transition-all text-sm font-label`}
                        style={{ background: colors.BackgroundSecondary, borderColor: colors.CardBorder, color: colors.TextBody }}
                      />
                    </div>
                  ...
                  <div className={`mt-4 ${styles.formField}`}>
                    <div className="flex justify-between items-center mb-0">
                      <label className="font-label text-[10px] uppercase tracking-widest font-bold block" style={{ color: colors.TextBody }}>
                        Query Parameters (JSON string) <span className="text-red-500">*</span>
                      </label>
                      <label className="px-3 py-1.5 rounded-xl text-[10px] font-headline font-bold uppercase tracking-wider transition-all duration-200 cursor-pointer flex items-center gap-1.5 border hover:opacity-80" style={{ background: colors.Background, borderColor: colors.CardBorder, color: colors.TextHeading }}>
                          ...
                          onChange={(e) => {
                            ...
                            reader.onload = (event) => {
                              try {
                                ...
                                showToast("JSON uploaded and validated successfully!", "success");
                              } catch (err) {
                                showToast("Invalid JSON file content. Please check the format.", "error");
                              }
                            };
                    ...
                    <div className="relative">
                      ...
                      <textarea
                        ...
                        onChange={(e) => updateApiField(api.id, 'apiQueryParams', e.target.value)}
                        className={`block w-full pl-12 pr-4 py-3 rounded-xl outline-none transition-all text-sm font-label`}
                        style={{ background: colors.BackgroundSecondary, borderColor: colors.CardBorder, color: colors.TextBody }}
                      />
                    </div>
        ...
        <div className="mt-4 flex flex-col sm:flex-row justify-between items-center gap-4 pt-3" style={{ borderTop: `1px solid ${colors.Border}` }}>
          ...
          <div className="flex flex-row items-center gap-4 w-full sm:w-auto justify-center">
            ...
            <button
              ...
              style={{ background: `linear-gradient(120deg, ${colors.ButtonGradientOne}, ${colors.ButtonGradientTwo})`, opacity: isStepTwoPending ? 0.7 : 1 }}
