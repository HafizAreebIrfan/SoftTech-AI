...
import { LayoutGridIcon, TerminalIcon, SlidersIcon, DatabaseIcon, CheckIcon, LeftArrowIcon } from "../../../../assets/icons";
...
const SignupStep3: FC = () => {
  ...
  const { companyId, selectedLayout, setSelectedLayout, setAuth } = useAuthStore();

  const { mutate: stepThreeMutate, isPending: isStepThreePending } = useMutation({
    mutationFn: ({ id, uiPreference }: { id: string; uiPreference: { layout: string } }) => saveCompanyUiSelection(id, uiPreference),
    onSuccess: (res) => {
        ...
        <div className="mt-4 flex flex-col sm:flex-row justify-between items-center gap-4 pt-3" style={{ borderTop: `1px solid ${colors.Border}` }}>
          ...
          <div className="flex flex-row items-center gap-4 w-full sm:w-auto justify-center">
            ...
            <button
              ...
              style={{ background: `linear-gradient(120deg, ${colors.ButtonGradientOne}, ${colors.ButtonGradientTwo})`, opacity: isStepThreePending ? 0.7 : 1 }}
