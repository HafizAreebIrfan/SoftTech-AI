...
import { OAuthConnectionRepository } from "../../../adapters/persistence/mongo/oauth/oauthConnectionRepository";
...
export class OAuthTokenService {
  private static tokenCache: Map<string, TokenCacheEntry> = new Map();
  private static readonly SAFETY_BUFFER_MS = 60 * 1000; // 60 seconds

  /**
   * Generates a cache key based strictly on tokenUrl and clientId.
   * Never uses clientSecret directly as a cache key.
   */
  private static getCacheKey(tokenUrl: string, clientId: string): string {
   ...
   */
  public static validateConfig(oauth?: IOAuth, apiName?: string): void {
  ...
  public static async getAccessToken(
    ...
    this.validateConfig(oauth, apiName);

    const tokenUrl = oauth!.tokenUrl!.trim();
    const clientId = oauth!.clientId!.trim();
    const clientSecret = oauth!.clientSecret!.trim();
    const cacheKey = this.getCacheKey(tokenUrl, clientId);

    if (!forceRefresh) {
      const cached = this.tokenCache.get(cacheKey);
      const now = Date.now();

      if (cached && cached.expiresAt > now + this.SAFETY_BUFFER_MS) {
        return cached.accessToken;
      }
    }

    const tokenEntry = await this.requestNewToken(
      tokenUrl,
      clientId,
      clientSecret,
      apiName,
   ...
   */
  public static invalidateToken(tokenUrl?: string, clientId?: string): void {
  ...
  public static async exchangeAuthorizationCode(
    ...
    return await OAuthConnectionRepository.saveUserConnection(connection);
  }
  ...
  public static async getUserAccessToken(
    ...
    const connection = await OAuthConnectionRepository.findUserConnection(
      companyId,
      apiId,
      userId,
    );
    ...
    const isExpired =
      ...
      new Date(connection.expiresAt).getTime() <= now + this.SAFETY_BUFFER_MS;
    ...
    if (connection.refreshToken && oauth?.tokenUrl && oauth?.clientId) {
      try {
        const refreshedConnection = await this.refreshUserAccessToken(
          connection,
          oauth,
        );
        return refreshedConnection.accessToken;
      } catch {
        // If refresh fails and token is expired, return null to trigger re-auth
        if (isExpired) {
          return null;
        }
        return connection.accessToken;
   ...
   */
  public static async refreshUserAccessToken(
    ...
    return await OAuthConnectionRepository.saveUserConnection(
      updatedConnection,
    );
  }
