...
exports.registerCompanyApiTools = void 0;
const server_1 = require("@modelcontextprotocol/ext-apps/server");
const genericwidgetoutputschema_1 = require("../../Schemas/OutputSchema/genericwidgetoutputschema");
const genericwidgetinputschema_1 = require("../../Schemas/InputSchema/genericwidgetinputschema");
const genericwidgetnormalizer_1 = require("./genericwidgetnormalizer");
const registerCompanyApiTools = (server, company) => {
    const apis = company.apis ?? [];
    apis.forEach((api, index) => {
        const toolName = toToolName(api.mcpToolName || api.name || `api_${index + 1}`, index);
        (0, server_1.registerAppTool)(server, toolName, {
            title: api.name || `API ${index + 1}`,
            description: api.mcpDescription ||
                `Calls ${api.name || "a registered company API"} and returns a generic widget response.`,
            inputSchema: genericwidgetinputschema_1.genericWidgetInputSchema,
            outputSchema: genericwidgetoutputschema_1.genericWidgetOutputSchema,
            _meta: {
                ui: {
            ...
            const widgetContent = (0, genericwidgetnormalizer_1.normalizeApiResponseToWidget)(api.name || company.companyName, rawResponse, company.uiPreference?.layout);
...
exports.registerCompanyApiTools = registerCompanyApiTools;
const callRegisteredApi = async (api, input) => {
    ...
    if (!response.ok) {
        throw new Error(`Registered API "${api.name}" failed with status ${response.status}`);
    }
    return response.json();
};
const buildApiUrl = (api, input) => {
